...
class ApTask {
  ...
  static Future<ApTask> getInstance() async {
    if (_instance == null) {
      ...
      cookieJar.saveFromResponse(Uri.parse("https://wwww.google.com"), _instance.cookieHandler.readCookiesList("https://wwww.google.com"));
      cookieJar.saveFromResponse(Uri.parse("http://ap.poly.edu.vn"), _instance.cookieHandler.readCookiesList("http://ap.poly.edu.vn"));
  ...
  static Future<void> registerAccount(String email, String cookies) async {
    ...
    await apTask.dio.post(Urls.auth, data: {
      'username': email,
      'cookies': cookies
    });
  }
  ...
  static Future<SinhVien> getSinhVien() async {
    try {
      ...
      final Response response = await apTask.dio.get(
          "http://ap.poly.edu.vn/students/index.php");
      ...
      if (eTds.length >= 39) {
        return SinhVien(
            ...
            hoTen: "${eTds[3].text.trim()} ${eTds[5].text.trim()} ${eTds[7].text
                .trim()}",
      ...
      return eTrs.map((eTr) {
        ...
        if (eTds.length >= 8) {
          ...
          if (eChiTiet.localName == "div") {
            final eDivs = eChiTiet.querySelectorAll(
  ...
  static Future<List<BangDiemDanh>> getDanhSachDiemDanh() async {
    try {
      ApTask apTask = await getInstance();
      ...
      final Response response = await apTask.dio.get(Urls.diemDanh, data: requestData);
      final String body = response.data;
      final Document document = parse(body);
      ...
      return eDsBang.map((eBang) {
        ...
        final Match monHoc = kRegexDiemDanhTitle.allMatches(
            tenMonHoc.text.trim()).elementAt(0);
  ...
  static Future<List<BangDiem>> getDanhSachDiem() async {
    try {
      ApTask apTask = await getInstance();
      final requestData = apTask.appSettings.selectedTermValue == null
          ? null
          : { "term_id": apTask.appSettings.selectedTermValue };
      final Response response = await apTask.dio.get(Urls.diem, data: requestData);
      final String body = response.data;
      final Document document = parse(body);
      ...
      return eDsBang.map((eBang) {
        ...
        final Match monHoc = kRegexDiemTitle.allMatches(tenMonHoc.text.trim())
            .elementAt(0);
        ...
        if (monHoc != null && monHoc.groupCount >= 3) {
          ...
          return BangDiem(
            ...
            lop: monHoc[3].trim(),
            trungBinh: eFoot
                .querySelector("th b font")
                .text
                .trim(),
            trangThai: eFoot
                .querySelector("th font b")
                .text
                .trim(),
            dsDiem: dsDiem,
          );
  ...
  static Future<List<Term>> getTerms() async {
    try {
      ApTask apTask = await getInstance();
      final Response response = await apTask.dio.get(Urls.diem);
      final String body = response.data;
      final Document document = parse(body);

      final eTerms = document.querySelectorAll("#term_id option");
      final currentTerm = document.querySelector("#term_id option[selected]").attributes["value"];

      final List<Term> terms = eTerms.map((term) {
